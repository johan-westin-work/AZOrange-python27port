...
class installer:
	...
	self.addLog("#Installation will be to made to " + self.AZOrangeInstallDir)
        ...
        cmd = 'python setup.py build %s %s %s "%s" %s "%s" "%s" %s' % (self.platformType,self.buildDir,self.AZOrangeInstallDir,self.InstallTimeModules , envTmp , self.dependencies,self.GCCmoduleForAppspackMPI,self.openInstallation)
        self.addLog("About to execute: " + cmd)
        self.addLog(commands.getstatusoutput(cmd))
